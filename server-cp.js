...
async function getCyberArkSecret(objectName) {
    // Pfad zum SDK (Standardpfad oder via ENV)
    const sdkPath = process.env.CYBERARK_SDK_PATH || '"C:\\Program Files (x86)\\CyberArk\\ApplicationPasswordSdk\\CLIPasswordSDK.exe"';
    const appId = process.env.CYBERARK_APP_ID || "PXM_Manager";
    const safe = process.env.CYBERARK_SAFE || "PXM_Safe";
    const folder = process.env.CYBERARK_FOLDER || "Root";

    const cmd = `${sdkPath} GetPassword -p AppDescs.AppID=${appId} -p Query="Safe=${safe};Folder=${folder};Object=${objectName}" -o Password`;
    
    try {
        const { stdout } = await execAsync(cmd);
        return stdout.trim();
    } catch (e) {
        console.error(`[CyberArk] Error retrieving secret for object '${objectName}':`, e.message);
        throw e;
    }
}
...
async function startServer() {
    try {
        console.log("[Startup] Retrieving credentials from CyberArk...");
        
        // Namen der Objekte im CyberArk Safe (können via .env überschrieben werden)
        const dbSecretObj = process.env.CYBERARK_OBJ_DB || "SQL_PXM_User";
        const adSecretObj = process.env.CYBERARK_OBJ_AD || "SVC_PXM_AD_Read";

        const dbPassword = await getCyberArkSecret(dbSecretObj);
        const adPassword = await getCyberArkSecret(adSecretObj);
        ...
        adConfig = {
            url: process.env.AD_URL || 'ldap://192.168.178.10',
            baseDN: process.env.AD_BASE_DN || 'dc=lab,dc=local',
        };

        AD_SERVICE_USER = process.env.AD_SERVICE_USER || 'svc_pxm_ad_read';
        AD_SERVICE_PASS = adPassword;
